...
import { analyzeImport } from '../analyzers/import';
import { AnalyzeWatchDecleration, AnalyzeWatchExpression } from '../analyzers/watch';
...
export async function parseScript(fileName: string, scriptBlock: SFCScriptBlock | null): Promise<ScriptInfo | null> {
    ...
    for (const body of program.body) {
        switch (body.type) {
            ...
            {
                imports.push(analyzeImport(body, content));
                break;
            }
                        ...
                        case 'CallExpression': {
                            ...
                            if (callee.type === 'Identifier') {
                                switch (callee.name) {
                                    case 'watch':
                                        watchers.push(
                                            AnalyzeWatchDecleration(declaration, content),
                                        );
                                        break;
                            ...
                            case 'Identifier':
                                {
                                    ...
                                    switch (name) {
                                        case 'watch':
                                            ...
                                            watchers.push(
                                                AnalyzeWatchExpression(expression, content),
                                            );
                                            break;
